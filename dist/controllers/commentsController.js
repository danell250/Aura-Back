...
exports.commentsController = {
    ...
    createComment: (req, res) => __awaiter(void 0, void 0, void 0, function* () {
        try {
            ...
            const mentionedHandles = (text.match(/@(\w+)/g) || []).map((h) => h.substring(1));
            if (mentionedHandles.length > 0) {
                const db = (0, db_1.getDB)();
                // Find users with these handles
                const taggedUsers = yield db.collection('users').find({
                    handle: { $in: mentionedHandles.map((h) => new RegExp(`^@?${h}$`, 'i')) }
                }).toArray();
                // Check if any tagged user has disabled tagging
                const blockedTags = taggedUsers.filter((user) => {
                ...
                if (blockedTags.length > 0) {
                    const blockedHandles = blockedTags.map((u) => u.handle).join(', ');
