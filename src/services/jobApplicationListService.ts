import {
  toApplicationCompanySummaryResponse,
  toApplicationResponse,
} from './jobApplicationResponseService';
...
const toApplicantJobApplicationListItem = (application: any) => ({
  ...toApplicationResponse(application),
  job: application?.job ? toJobResponse(application.job) : null,
  company: toApplicationCompanySummaryResponse(application?.company),
});

export const listApplicantJobApplications = async (params: {
  db: any;
  applicantUserId: string;
  status?: string;
  skip: number;
