...
export const usersController = {
    ...
    try {
      ...
      if (normalizedFeaturedPostIds.length > 0) {
        ...
        const ownsEveryPost = ownedPosts.length === normalizedFeaturedPostIds.length
          && ownedPosts.every((post: any) => String(post?.author?.id || '') === authUserId)
          && ownedPosts.every((post: any) => {
            const ownerType = String(post?.ownerType || '').trim().toLowerCase();
  ...
  cancelConnectionRequest: async (req: Request, res: Response) => {
    try {
      ...
      const targetUser = await db.collection('users').findOne({ id: targetUserId }, { projection: { id: 1 } });
      if (!targetUser) {
        return res.status(404).json({
          success: false,
  ...
  getUserByHandle: async (req: Request, res: Response) => {
    try {
  ...
  searchUsers: async (req: Request, res: Response) => {
    try {
      ...
      const buildTokenClauses = (fields: string[]) =>
        tokenRegexes.map((tokenRegex) => ({
      ...
      const [usersResults, companiesResults, exactUserByHandle, exactCompanyByHandle] = await Promise.all([
        db.collection('users')
          .find(usersQuery)
          .project({
            id: 1,
      ...
      const transformedUsers = usersResults.map((u) =>
      ...
      const exactUserResult = exactUserByHandle
        ? sanitizePublicUserProfile({
            ...transformUser(exactUserByHandle),
            type: 'user'
          })
        : null;
      ...
      const exactCompanyResult = exactCompanyByHandle
        ? (() => {
            const transformedCompany = transformUser({
              ...exactCompanyByHandle,
              type: 'company',
              companyName: exactCompanyByHandle.name,
              companyWebsite: exactCompanyByHandle.website,
              userMode: 'company'
            });
      ...
      const dedupedResults = new Map<string, any>();
  ...
  sendConnectionRequest: async (req: Request, res: Response) => {
    try {
      ...
      const newNotification = await createNotificationInDB(
        id,
