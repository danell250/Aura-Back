...
import { recordJobPulseEventsAsync } from '../services/jobPulseService';
import { enqueueReverseJobMatchJobs } from '../services/reverseJobMatchQueueService';
    ...
    try {
      ...
      if (stats.insertedJobIds.length > 0) {
        recordJobPulseEventsAsync(
          db,
          stats.insertedJobIds.map((jobId) => ({
            jobId,
      ...
      if (stats.insertedJobIds.length > 0) {
        await enqueueReverseJobMatchJobs({
          db,
