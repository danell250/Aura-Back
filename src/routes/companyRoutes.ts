...
const normalizeCompanyName = (value: unknown): string | null => {
  ...
  if (!cleaned || cleaned.length > COMPANY_NAME_MAX_LENGTH) return null;
  if (COMPANY_NAME_URL_PATTERN.test(cleaned)) return null;
  if (COMPANY_NAME_HTML_TAG_PATTERN.test(cleaned)) return null;
  if (!COMPANY_NAME_ALLOWED_PATTERN.test(cleaned)) return null;
  return cleaned;
};

const checkInviteRateLimit = async (
  ...
  const oldestInWindow = recentInvites[recentInvites.length - 1]?.createdAt;
...
const getDefaultCompanyPrivacySettings = () => ({
  ...
  allowDirectMessages: 'everyone' as 'everyone' | 'friends' | 'none',
...
const normalizeCompanyPrivacySettings = (incoming: any, base?: any) => {
  const source = {
    ...getDefaultCompanyPrivacySettings(),
...
router.get(COMPANY_PRIVACY_SETTINGS_ROUTE, requireAuth, async (req, res) => {
  try {
    const { companyId } = req.params;
    const currentUser = (req as any).user;
    const db = getDB();
    ...
    const privacySettings = normalizeCompanyPrivacySettings(company?.privacySettings);
    const profileViewIds = normalizeUniqueIds(company?.profileViews);
...
router.put(COMPANY_PRIVACY_SETTINGS_ROUTE, requireAuth, async (req, res) => {
  try {
    const { companyId } = req.params;
    const currentUser = (req as any).user;
    ...
    const company = access.company;
    const updatedSettings = normalizeCompanyPrivacySettings(settings, company?.privacySettings || {});
...
router.post('/:companyId/invites', requireAuth, async (req, res) => {
  try {
    ...
    const inviteRateLimit = await checkInviteRateLimit(db, currentUser.id);
